...
public class UserUpdateServletTest {
    ...
    public void updateUser() throws ServletException, IOException {
        ...
        validate.addValidate(new UserBuilder().setName("Petr Arsentev").setLogin("Petr").build());
        mockStatic(ValidateService.class);
        Mockito.when(ValidateService.getInstance()).thenReturn(validate);
        HttpServletRequest req = mock(HttpServletRequest.class);
        HttpServletResponse resp = mock(HttpServletResponse.class);
        when(req.getParameter("id")).thenReturn("1");
        when(req.getParameter("name")).thenReturn("Petr Arsentev Job4j");
        when(req.getParameter("login")).thenReturn("Arsentev");
        new UserUpdateServlet().doPost(req, resp);
        assertThat(validate.findAllValidate().iterator().next().getName(), is("Petr Arsentev Job4j"));
        assertThat(validate.findAllValidate().iterator().next().getLogin(), is("Arsentev"));
    }
}
